...
class Movie:
    def __init__(self, row):
        self.id = row['id']
        self.short = row['short_name']
        self.long = row['long_name']
    ...
    def __str__(self):
        return str(self.id) + ', '  + self.short + ', ' + self.long
...
class BinaryNode:
    ...
    def compareTo(self, value):
        ...
        """
        if self.value == Movie(value):
            return 0
        if self.value < Movie(value):
            return -1
        return +1

    def add(self, val, countRotation):
        ...
        if self.compareTo(val) >= 0:
            self.left = self.addToSubTree(self.left, val, countRotation)
            ...
            self.right = self.addToSubTree(self.right, val, countRotation)
            if self.heightDifference() == -2:
...
class BinaryTree:
    ...
    def add(self, value, countRotation = False):
